...
\end{itemize}
Un'\defterm{immersione isometrica} è un'isometria fra spazi seminormati (osserviamo che un'isometria non è necessariamente iniettiva).
...
Sia $\map{f}{M}{N}$ una funzione continua fra $n$-varietà orientate \todo{Si può parlare di grado fra varietà non orientate?}, e sia $d$ il grado di $f$. Allora $|d|\cdot\norm{N}\le\norm{M}$.
\end{proposition}
\begin{proof}
...
Data una $n$-varietà chiusa orientata, esiste un'unica classe in coomologia $[M]^*\in H^n(M)$ tale che $\langle[M]^*,[M]\rangle=1$; $[M]^*$ è detta \defterm{coclasse fondamentale} di $M$.
