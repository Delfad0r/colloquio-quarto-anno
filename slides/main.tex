...
Sia $M$ una varietà Riemanniana chiusa\only<3->{ \underline{con curvatura non positiva}}.\\
Allora il rapporto
\[
...
\]
\tikz[overlay,remember picture,]\draw[opacity=.5,gray,-latex,visible on=<4>,shorten <=5pt,shorten >=5pt] (euclidean manifolds 1.east) to[out=0,in=0,looseness=2] (euclidean manifolds 2.east);
\onslide<+(-1)->{da cui $\norm{M}=0$.}
\end{itemize}
\end{frame}
...
\]
\tikz[overlay,remember picture,visible on=<3>]\draw[gray!50,-latex] ++(maps hyperbolic manifolds equal) +(-1,-1.2) node[anchor=east,gray] {principio di proporzionalità} to[bend right] +(0,-5pt);
\end{block}
\end{frame}
...
\begin{itemize}
\item<2-> Per $g\le 1$ vale $\norm{\Sigma_g}=0$. \onslide<2>{\tikz[baseline=-.7ex]\draw[gray!50,-latex] (1,0) -- (0,0); {\color{gray}$\Sigma_0=S^2$, $\Sigma_1=S^1\times S^1$}}
\item<3-> Per $g\ge 2$, \alt<3-7>{$\Sigma_g$ ammette una metrica iperbolica.

\vspace{1cm}
\[
\onslide<4-7>{\norm{\Sigma_g}=\frac{\tikzmarknode{closed surfaces num}{\Vol(\Sigma_g)}}{\tikzmarknode{closed surfaces den}{v_2}}}\onslide<6-7>{=-\frac{2\pi\chi(\Sigma_g)}{\pi}}\onslide<7>{=4g-4}
\]
\begin{tikzpicture}[remember picture,overlay]
\draw[gray!50,-latex,visible on=<5-7>] ++(closed surfaces num) +(0,9pt) to[bend left] +(1,1) node[gray,anchor=west] {Gauss-Bonnet};
\draw[gray!50,-latex, visible on=<4-7>] ++(closed surfaces den) +(0,-6pt) to[bend right] +(1,-1) node[gray,anchor=west,align=left] {area del triangolo\\ideale in $\HH^2$};
\end{tikzpicture}
}{$\norm{\Sigma_g}=4g-4$.}
\item<8-> Sia $\map{f}{\Sigma_{g_1}}{\Sigma_{g_2}}$ con $g_1\ge1$, $g_2\ge 2$. \onslide<9->{Allora
\[
|\deg(f)|\le\frac{g_1-1}{g_2-1}\pause=\frac{\chi(\Sigma_{g_1})}{\chi(\Sigma_{g_2})}.
\]
}
...
\langle[M]^*,[M]\rangle=1.
\]
\tikz[overlay,remember picture,visible on=<2>]\draw[gray!50,-latex] ++(fundamental coclass exists) +(-.5,-1) node[anchor=north,gray,align=left] {$H_n(M)\iso\RR$\\$H^n(M)\iso\RR$} to[out=60,in=-100] +(0,-7pt);
...
\item Sia $\Gamma=\pi_1(M)$ identificato con $\Aut(\widetilde M,p)$\only<3>{{\color{gray}$<\Isom^+(\widetilde M)$}}.
\end{itemize}

\onslide<+->{$\Gamma$ agisce sul complesso di cocatene $C^\bul(\widetilde M)$.}
\onslide<+->{
\begin{block}{Proposizione}
Il rivestimento $p$ induce isomorfismi isometrici
\begin{alignat*}{2}
p^\bul\colon&&C^\bul(M)&\longrightarrow C^\bul(\widetilde M)^\Gamma,\\
\onslide<+->{H^\bul(p^\bul)\colon&&H^\bul(M)&\longrightarrow H^\bul(C^\bul(\widetilde M)^\Gamma).}
\end{alignat*}
\end{block}}
\end{frame}
...
\draw[gray!50!black,postaction={decorate}] (x3) .. controls ++ (-85:2) and ++(130:2) .. (p);
\end{scope}
...
\onslide<+->{I simplessi dritti sono lisci.}
\end{frame}
...
\tikz[overlay,remember picture]\draw[gray!50,-latex,visible on=<2>] ++(volume cocycle stokes) +(-1,.8) node[gray,anchor=east] {Stokes} to[bend left] +(0,5pt);
...
\begin{frame}{\secname}{\subsecname}
\only<1-8>{
\onslide<1-7>{Sia $M$ una $n$-varietà iperbolica chiusa.}
\begin{itemize}
\item<2-7> Il rivestimento universale è $\widetilde M=\HH^n$.
\item<3-7> Obiettivo: stimare $\tikzmarknode{hyperbolic manifolds vol}{\norm{[\Vol_{\HH^n}]^G_c}_\infty}$.
\tikz[overlay,remember picture,visible on=<4-7>]\draw[gray!50,-latex] ++(hyperbolic manifolds vol.east) +(-4pt,2pt) to[out=-30,in=90] +(.3,-1) node[inner sep=-3pt,gray,anchor=north west] {$\begin{aligned}\Vol_{\HH^n}(s)&=\int_{\str_n(s)}\omega_{\HH^n}\onslide<7>{\le v_n}\end{aligned}$};
\item<5-7> I simplessi dritti sono \emph{geodetici}.
\end{itemize}
\vspace{.5cm}
}
\onslide<6->{
...
\only<9->{
\begin{itemize}
\item<10-> Per ogni $x\in C_n(\HH^n)$ vale
\[
|\Vol_{\HH^n}(c)|\le v_n\cdot\norm{c}_1\implies \norm{\Vol_{\HH^n}}_\infty\le v_n.
\]
\item<11-> Allora
\[
\tikzmarknode{hyperbolic manifolds vn}{\textstyle\norm{[\Vol_{\HH^n}]^G_c}_\infty}\le\norm{\Vol_{\HH^n}}_\infty\onslide<12->{\le v_n.}
\]
\tikz[overlay,remember picture,visible on=<13->]\draw[gray!50,-latex] ++(hyperbolic manifolds vn.south west) +(-2pt,7pt) to[bend right] +(-1.8,0) node[gray,anchor=north] {$\displaystyle\norm{M}\ge\frac{\Vol(M)}{v_n}$};
\end{itemize}
}
\end{frame}
\end{document}