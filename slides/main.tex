...
\coordinate (p) at (4,2);
\fill[style between={1}{3}{},style between={3}{4}{dashed},green!40!black,fill=green,fill opacity=.3]
(x0)  .. controls ++(20:3) and ++(170:3) ..
(x1) .. controls ++(160:2) and ++(-60:2) ..
(x2) .. controls ++(-100:2) and ++(30:2) .. cycle;
\begin{scope}[decoration={markings,mark=between positions .1 and 1 step .2 with {\arrow{stealth}}}]
...
\end{scope}
\foreach \c in {x0,x1,x2,x3,p} {\fill (\c) circle[radius=4pt];}
...
\draw[-latex,gray!50] (volume cocycle VolMtilde) to[out=-170,in=180,out looseness=2,in looseness=1.5] ([shift={(-4pt,4pt)}]pic cs:volume cocycle integral);
\end{onslide}
\end{frame}
