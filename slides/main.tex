...
\begin{itemize}
\item Per $g\le 1$ vale $\norm{\Sigma_g}=0$. {\color{gray}$\Sigma_0=S^2$, $\Sigma_1=S^1\times S^1$};
\item Per $g\ge 2$, $\Sigma_g$ ammette una metrica iperbolica.

\vspace{1cm}
\[
\norm{\Sigma_g}=\frac{\tikzmarknode[inner sep=3pt]{closed surfaces num}{\Vol(\Sigma_g)}}{\tikzmarknode[inner sep=3pt]{closed surfaces den}{v_2}}=-\frac{2\pi\chi(\Sigma_g)}{\pi}=4g-4
\]
\end{itemize}
\begin{tikzpicture}[remember picture,overlay]
\node[gray,anchor=west] (gauss-bonnet) at ($(closed surfaces num)+(1cm,1cm)$) {Gauss-Bonnet};
\node[gray,anchor=west,align=left] (ideal triangle) at ($(closed surfaces den)+(1cm,-1cm)$) {area del triangolo\\ideale in $\HH^2$};
\draw[gray!50,-latex] (closed surfaces num.north) to[bend left] (gauss-bonnet.west);
\draw[gray!50,-latex] (closed surfaces den.south) to[bend right] (ideal triangle.west);
\end{tikzpicture}
\end{frame}
...
\coordinate (p) at (4,2);
\fill[style between={1}{3}{},style between={3}{4}{dashed},green!40!black,fill=green!70!black,fill opacity=.3]
...
\fill[green!40!black] (p) circle[radius=3pt];
...
\item Vale $[\Vol_M]=\Vol(M)\cdot[M]^*$\tikzmark{volume cocycle VolM}.
\end{itemize}
\vspace{.5cm}
\[
\color{gray} \norm{M}=\frac{1}{\norm{[M]^*}_\infty}\overset{\tikzmark{volume cocycle equal}}{=}\frac{\Vol(M)}{\norm{[\Vol_M]}_\infty}
\]
\tikz[remember picture,overlay]\draw[gray!50,-latex] (pic cs:volume cocycle VolM) to[bend left=15] (pic cs:volume cocycle equal);
\end{frame}
\begin{frame}{\secname}{\subsecname}
...
\]
%\tikz[overlay,remember picture]\draw[-latex,gray!50] (volume cocycle VolMtilde) to[out=-170,in=180,out looseness=2,in looseness=1.5] ([shift={(-4pt,4pt)}]pic cs:volume cocycle integral);
\tikz[overlay,remember picture]\draw[-latex,gray!50] ($(volume cocycle VolMtilde.center)+(-5pt,-7pt)$) .. controls ++(-5,-.5) and ++(-3,0) .. ([shift={(-4pt,4pt)}]pic cs:volume cocycle integral);
\end{onslide}
\end{frame}
