...
\draw[gray!50,-latex] ++(maps hyperbolic manifolds equal) +(-1,-1.2) node[anchor=east,gray] {principio di proporzionalità} to[bend right] +(0,-5pt);
\draw[gray!50,-latex] ++(maps hyperbolic manifolds le) +(1,-1.2) node[anchor=west,gray] {$\norm{\Sigma_g}=4g-4$} to [bend left] +(0,-7pt);
\end{tikzpicture}
\end{frame}
...
\draw[smooth,tension=.5,opacity=#2,red!30!black,fill=red,fill opacity=.3*#2] ($(x0)+#1$) \foreach \p/\q in {x0/x1,x1/x2,x2/x0} { -- plot coordinates {($(\p)+#1$) ($(\p-1)+#1$) ($(\p-2)+#1$) ($(\p-3)+#1$) ($(\p-4)+#1$) ($(\p-5)+#1$) ($(\p-6)+#1$) ($(\p-7)+#1$) ($(\p-8)+#1$) ($(\p-9)+#1$) ($(\q)+#1$)} };}
\newcommand{\drawSmoothSimplex}[2]{
\draw[black,opacity=#2,green!30!black,fill=green,fill opacity=.4*#2]
...
\onslide<2->{\drawRoughSimplex{(0,0)}{.4}}
\onslide<2>{\drawRoughSimplex{(0,\h1)}{1}}
\onslide<3->{\drawRoughSimplex{(0,\h1)}{.4}}
\onslide<3>{\drawSmoothSimplex{(0,\h1)}{1}}
\onslide<4->{\drawSmoothSimplex{(0,\h1)}{.4}}
\onslide<4>{\drawSmoothSimplex{(0,0)}{1}}

\foreach \p in {x0,x1,x2} {
\fill[alt=<2-3>{red!30!gray}{alt=<1>{red!30!black}{green!30!black}}] (\p) circle[radius=2pt];
\onslide<2->{\fill[alt=<2>{red!30!black}{alt=<3>{green!30!black}{green!30!gray}}] ($(\p)+(0,\h1)$) circle[radius=2pt];}
}
\node (M) at ($(x1)+(3,2)$) {$M$};
\node (Mtilde) at ($(x1)+(3,2+\h1)$) {$\widetilde M$};
\draw[->,shorten <=10pt,shorten >=10pt] (Mtilde.south) -- (M.north) node[midway,right] {$p$};
\onslide<1-3>{\node[alt=<1>{opacity=1}{opacity=.5},red!40!black] (s) at ($(M)+(-17,0)$) {$s$};}
\onslide<2>{\node[alt=<2>{opacity=1}{opacity=.5},red!40!black] (stilde) at ($(s)+(0,\h1)$) {$\widetilde s$};}
\onslide<3-4>{\node[alt=<3>{opacity=1}{opacity=.5},green!40!black] at (stilde) {$[\widetilde s(e_0),\ldots,\widetilde s(e_k)]$};}
\onslide<4>{\node[green!40!black] at (s) {$\str_k(s)$};}
\end{tikzpicture}
\end{center}
\end{frame}
